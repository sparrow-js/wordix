...
// import type { AnnotationReply, MessageEnd, MessageReplace, ThoughtItem } from '@/components/base/chat/chat/type'
type AnnotationReply = any;
type MessageEnd = any;
type MessageReplace = any;
type ThoughtItem = any;
...
// } from '@/types/workflow'
type IterationFinishedResponse = any;
type IterationNextResponse = any;
type IterationStartedResponse = any;
type NodeFinishedResponse = any;
type NodeStartedResponse = any;
type ParallelBranchFinishedResponse = any;
type ParallelBranchStartedResponse = any;
type TextChunkResponse = any;
type TextReplaceResponse = any;
type WorkflowFinishedResponse = any;
type WorkflowStartedResponse = any;
                ...
                bodyJson
                  .then((data: ResponseError) => {
                    if (data.code === "init_validate_failed" && IS_CE_EDITION && !silent)
                      ...
                      console.log({ type: "error", message: data.message, duration: 4000 });
                    else if (data.code === "not_init_validated" && IS_CE_EDITION)
                      globalThis.location.href = `${globalThis.location.origin}/init`;
                    else if (data.code === "not_setup" && IS_CE_EDITION)
                      globalThis.location.href = `${globalThis.location.origin}/install`;
                    else if (location.pathname !== "/signin" || !IS_CE_EDITION) globalThis.location.href = loginUrl;
                    else if (!silent)
                      // Toast.notify({ type: 'error', message: data.message })
                      console.log({ type: "error", message: data.message });
                  })
                  .catch(() => {
