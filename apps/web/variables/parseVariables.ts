import { safeStringify } from "@/variables/lib/safeStringify";
import { isDefined, isNotDefined, isNotEmpty } from "@/variables/lib/utils";
...
export const parseVariables =
  ...
  (text: string | undefined): string => {
    if (!text || text === "") return "";
    const textWithInlineCodeParsed = text.replace(inlineCodeRegex, (_full, inlineCodeToEvaluate) => {
    ...
    return textWithInlineCodeParsed.replace(
      ...
      (_full, nameInCurlyBraces, _dollarSign, nameInTemplateLitteral) => {
        ...
        const variable = variables.find((variable) => {
          return matchedVarName === variable.name && (options.fieldToParse === "id" || isDefined(variable.value));
        }) as VariableWithValue | undefined;
        if (!variable) return dollarSign + "";
        if (options.fieldToParse === "id") return dollarSign + variable.id;
        const { value } = variable;
        if (options.isInsideJson) return dollarSign + parseVariableValueInJson(value);
        const parsedValue =
          dollarSign +
          safeStringify(options.takeLatestIfList && Array.isArray(value) ? value[value.length - 1] : value);
