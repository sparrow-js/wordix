...
export const isEmpty = (value: string | undefined | null): value is undefined | null =>
...
export const parseNumberWithCommas = (num: number) => num.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
...
export const isSvgSrc = (src: string | undefined) => src?.startsWith("data:image/svg") || src?.endsWith(".svg");
