...
class ApiClient {
  ...
  fetch = async <T = any>(
    ...
    try {
      response = await (options?.retry === false ? fetch : fetchWithRetry)(urlToFetch, {
        method,
        body,
        headers,
        redirect: "follow",
    ...
    if (response.status === 429) {
      throw new RateLimitExceededError(`Too many requests, try again in a minute.`);
    }
  ...
  get = <T = any>(path: string, data: JSONObject | undefined, options?: FetchOptions) =>
    this.fetch<T>(path, "GET", data, options);

  post = <T = any>(path: string, data?: JSONObject | FormData | undefined, options?: FetchOptions) =>
