...
export default function download(data: Blob | string | File, strFileName: string, strMimeType?: string) {
  const self = window;
  ...
  if (self.URL) {
    ...
    if (blob && (typeof blob === "string" || blob.constructor === z) && typeof m === "string") {
      try {
