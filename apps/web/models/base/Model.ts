...
export default abstract class Model {
   ...
   */
  async loadRelations(options: { withoutPolicies?: boolean } = {}): Promise<any> {
    const relations = getRelationsForModelClass(this.constructor as typeof Model);
    if (!relations) {
      return;
    }
    ...
    for (const properties of relations.values()) {
      const store = this.store.rootStore.getStoreForModelName(properties.relationClassResolver().modelName);
      if ("fetch" in store) {
  ...
  updateData: any = action((data: Partial<Model>) => {
    if (this.initialized) {
    ...
    if (!this.initialized) {
      LifecycleManager.executeHooks(this.constructor, "afterChange", this, previousAttributes);
    }
  });
  ...
  toJSON() {
    ...
    for (const property in this) {
      if (
        ...
        !["persistedAttributes", "store", "isSaving", "isNew"].includes(property)
      ) {
  ...
  isDirty(): boolean {
    ...
    return JSON.stringify(this.persistedAttributes) !== JSON.stringify(attributes);
  }
