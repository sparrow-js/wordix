...
}: PortalToFollowElemOptions = {}): ReturnType<typeof useFloating> & {
...
export function usePortalToFollowElemContext(): NonNullable<ContextType> {
...
export function PortalToFollowElem({
  ...
  return <PortalToFollowElemContext.Provider value={tooltip}>{children}</PortalToFollowElemContext.Provider>;
}
...
export const PortalToFollowElemContent = React.forwardRef<HTMLDivElement, React.HTMLProps<HTMLDivElement>>(
  ({ style, ...props }, propRef) => {
    const context = usePortalToFollowElemContext();
