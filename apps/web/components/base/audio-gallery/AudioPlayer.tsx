...
const AudioPlayer: React.FC<AudioPlayerProps> = ({ src }) => {
  ...
  const drawWaveform = useCallback(() => {
    ...
    data.forEach((value, index) => {
      ...
      if (index * barWidth <= playedWidth) color = "#296DFF";
      else if (((index * barWidth) / width) * duration <= hoverTime) color = "rgba(21,90,239,.40)";
      else color = "rgba(21,90,239,.20)";
  ...
  return (
    <div className={styles.audioPlayer}>
      ...
      <div
        className={isAudioAvailable ? styles.audioControls : styles.audioControls_disabled}
