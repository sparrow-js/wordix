...
export const getFileExtension = (fileName: string, fileMimetype: string) => {
  if (fileName) {
    ...
    if (fileNamePairLength > 1) return fileNamePair[fileNamePairLength - 1];
  }

  if (fileMimetype) return mime.getExtension(fileMimetype) || "";
...
export const getFileAppearanceType = (fileName: string, fileMimetype: string) => {
  ...
  if (extension === "md" || extension === "markdown") return FileAppearanceTypeEnum.markdown;
...
export const getProcessedFilesFromResponse = (files: any[]) => {
...
export const getSupportFileExtensionList = (allowFileTypes: string[], allowFileExtensions: string[]) => {
  ...
  return allowFileTypes.flatMap((type) => FILE_EXTS[type]);
};
...
export const getFilesInLogs = (rawData: any) => {
  const originalFiles = flatten(
    Object.keys(rawData || {})
      .map((key) => {
        if (typeof rawData[key] === "object" || Array.isArray(rawData[key])) return rawData[key];
        return undefined;
      })
      .filter(Boolean),
  ).filter((item) => item?.model_identity === "__dify__file__");
