...
import { SubscriptionAlerts } from "@/components/subscriptions/components/subscription-alerts";
import { SubscriptionHeaderActionButton } from "@/components/subscriptions/components/subscription-header-action-button";
...
export function SubscriptionHeader({ subscription }: Props) {
  ...
  const price =
    subscriptionItem.quantity * Number.parseFloat(subscription?.recurringTransactionDetails?.totals.total ?? "0");
